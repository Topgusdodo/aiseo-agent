...
aiseo cron create \
  '0 8 * * 1' \
  'Run seo-weekly-report on the primary site stored in MEMORY.md. Produce the 3-section delta report (current snapshot / changes vs last / next actions). If MEMORY.md has no primary site, ask the user to set one and skip this run.' \
...
aiseo cron create \
  '0 9 1 * *' \
  'Run technical-seo-audit on the primary site stored in MEMORY.md (site_root = primary site origin). Use focus=all and sample_pages=3. Produce the 3-section technical audit report. If MEMORY.md has no primary site, ask the user and skip this run.' \
...
aiseo cron create \
  '0 10 1 * *' \
  "Run keyword-opportunity for the top keyword from MEMORY.md '## 关注关键词' section (seed_keyword = first entry). Use market and language from MEMORY.md '## 目标市场 / 受众' (default US/en if absent). Produce the 3-section opportunity report. If MEMORY.md has no tracked keyword, ask the user and skip this run." \
...
aiseo cron create \
  '0 10 1 */3 *' \
  "Run competitor-analysis with user_url = primary site from MEMORY.md and competitor_urls = first 3 entries from MEMORY.md '## 主要竞品'. Use focus=all. Produce the 3-section comparison report with delta matrix. If MEMORY.md has < 2 competitors set, ask the user and skip this run." \
