...
TARGETED_RUNNER = REPO_ROOT / "scripts" / "run_targeted_smoke.sh"
...
def test_smoke_runners_default_to_web_only_toolsets():
    runner = RUNNER.read_text()
    targeted = TARGETED_RUNNER.read_text()
    for body in (runner, targeted):
        assert 'SMOKE_TOOLSETS="${AISEO_SMOKE_TOOLSETS:-web}"' in body
        assert 'chat --toolsets "$SMOKE_TOOLSETS" -q "$prompt" --quiet' in body
...
def test_grade_filters_log_by_session_id(tmp_path):
    grade = _load_grade_module()
    sid = "S6-04"
    _write_result_with_meta(
        tmp_path,
        sid,
        """## 基础元数据
seo-weekly-report 首次运行。
...
""",
        log_text="\n".join(
            [
                "2026-05-15 12:00:00 INFO [alien] run_agent: tool terminal completed",
                "2026-05-15 12:00:01 INFO [target] run_agent: conversation turn: session=target model=x",
            ]
        ),
        meta_extra="session_id=target\n",
    )
    ...
    assert result.tool_dispatches == ()


def test_grade_classifies_timeout_and_api_connection_as_infra(tmp_path):
    grade = _load_grade_module()
    sid = "S6-01"
    _write_result_with_meta(
        tmp_path,
        ...
        "",
        log_text=(
            "2026-05-15 12:00:00 WARNING [target] run_agent: "
            "API call failed error_type=APIConnectionError summary=Connection error."
        ),
        meta_extra="final_rc=124\nsession_id=target\n",
    )
    ...
    assert result.fail_reason == "infra_error"


def test_grade_does_not_classify_successful_retry_timeout_noise_as_infra(tmp_path):
    grade = _load_grade_module()
    sid = "S6-04"
    _write_result_with_meta(
        tmp_path,
...
""",
        log_text="\n".join(
            [
                "2026-05-15 12:00:00 INFO [target] tools.browser_tool: Started inactivity cleanup thread (timeout: 120s)",
                "2026-05-15 12:00:01 DEBUG [target] run_agent: Error classified: reason=timeout status=None retryable=True",
            ]
        ),
        meta_extra="attempts=2\nfinal_rc=0\nflaky=true\nsession_id=target\n",
    )
...
def test_grade_s6_all_infra_is_non_accepting(tmp_path):
    grade = _load_grade_module()
    sids = ("S6-01", "S6-02", "S6-03", "S6-04")
    for sid in sids:
        _write_result_with_meta(
            tmp_path,
            sid,
    ...
    overall, diags = grade.evaluate_acceptance(grade.summarize(results))
    assert not overall
    assert any("S6 INFRA-BLOCKED: 0/4 executable (infra=4)" in d for d in diags)
    assert not any("S6 OK: 0/0" in d for d in diags)


def test_grade_uses_final_successful_attempt_for_budget(tmp_path):
    grade = _load_grade_module()
    sid = "S6-04"
    _write_result_with_meta(
        tmp_path,
...
""",
        log_text="\n".join(
            [
                "2026-05-15 12:00:00 INFO [old] run_agent: tool browser_navigate completed",
                "2026-05-15 12:00:01 INFO [old] run_agent: tool browser_snapshot completed",
                "2026-05-15 12:00:02 INFO [old] run_agent: tool browser_console completed",
                "2026-05-15 12:00:03 INFO [old] run_agent: tool browser_vision completed",
                "2026-05-15 12:00:04 INFO [old] run_agent: tool browser_scroll completed",
                "2026-05-15 12:00:05 INFO [old] run_agent: tool browser_navigate completed",
                "2026-05-15 12:01:00 INFO [final] run_agent: tool web_extract completed",
                "2026-05-15 12:01:01 INFO [final] run_agent: tool web_extract completed",
            ]
        ),
    ...
    assert result.passed
    assert result.tool_dispatches == ("web_extract", "web_extract")
